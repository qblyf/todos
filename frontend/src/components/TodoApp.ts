import { Todo, TodoAppState } from '../types/todo.js';
...
export default class TodoApp {
  ...
  // 状态管理
  private setState(newState: Partial<TodoAppState>): void {
  ...
  private async handleCreateTodo(description: string): Promise<void> {
    try {
  ...
  private async handleToggleTodo(id: number): Promise<void> {
    try {
      // 乐观更新：立即更新本地状态
      ...
      notifications.success(
        updatedTodo.completed ? '任务已完成' : '任务已重新激活',
  ...
  private async handleDeleteTodo(id: number): Promise<void> {
  ...
  private async handleEditTodo(id: number, description: string): Promise<void> {
